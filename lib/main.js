...
exports.timeslot_to_time = function(timeslot_val)
{
   ...
   timeConv = Math.floor(timeConv).toString();

   var minutes = (timeConv - Math.floor(timeConv))*60;
   minutes = Math.floor(minutes).toString();
   if (minutes.length == 1) minutes = minute + "0";

   var time = timeConv + ":" + minutes;

   return time;
}

exports.time_to_timeslot = function(time_val)
{
    var hours = time_val.split(":")[0];
    var minutes = time_val.split(":")[1];

    var timeslot = (parseInt(hours)*4) + ((parseInt(minutes)/60)*4);
